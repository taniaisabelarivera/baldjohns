...
export default function DivePage() {
  ...
  const bubbleCount = 15; // Number of bubbles floating at once

  // --- 3. THE SCROLL ENGINE ---
  const { scrollY } = useScroll();
  const [currentDepth, setCurrentDepth] = useState(0);

  // Main background gradient transition
  ...
  // Scenic: Fades out the header and sunlight rays
  const introOpacity = useTransform(scrollY, [0, 400], [1, 0]);
  const rayOpacity = useTransform(scrollY, [0, 600], [0.6, 0]);
  ...
  return (
    <motion.main style={{ 
      ...
      {/* --- SCENIC: SUNLIGHT RAYS (Only at the surface) --- */}
      <motion.div style={{
        position: 'fixed', top: 0, left: 0, right: 0, height: '800px',
        background: 'linear-gradient(105deg, transparent 20%, rgba(255,255,255,0.15) 40%, rgba(255,255,255,0.1) 50%, transparent 70%)',
        filter: 'blur(60px)',
        opacity: rayOpacity,
        pointerEvents: 'none',
        zIndex: 1
      ...
      {Array.from({ length: bubbleCount }).map((_, i) => (
        <motion.div
          ...
          animate={{ 
            y: '-10vh',
            x: [`${Math.random() * 100}vw`, `${Math.random() * 100 + 2}vw`, `${Math.random() * 100 - 2}vw`]
          }}
          transition={{
        ...
        return (
          <motion.div 
            ...
            style={{ 
              ...
              zIndex: 20,
              boxShadow: `0 0 40px ${isVerified ? 'rgba(0, 255, 170, 0.25)' : 'rgba(0, 212, 255, 0.15)'}`,
